    ...
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-gray-500 text-sm mt-1">Registra un nuevo usuario y asígnale sus permisos.</p>
            </div>
        </div>
    </x-slot>
            ...
            <form action="{{ route('users.store') }}" method="POST">
                ...
                <div class="p-8 sm:p-12 space-y-10">
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="text" id="name" name="name" value="{{ old('name') }}" class="w-full border-gray-300 focus:border-blue-500 focus:ring-blue-500 rounded-xl shadow-sm bg-gray-50 px-4 py-2.5" required autofocus>
                            ...
                            <div>
                                ...
                                <input type="email" id="email" name="email" value="{{ old('email') }}" class="w-full border-gray-300 focus:border-blue-500 focus:ring-blue-500 rounded-xl shadow-sm bg-gray-50 px-4 py-2.5" required>
                    ...
                    <div>
                        ...
                        <div class="mb-8" x-data="{ currentRole: '{{ old('role', 'usuario') }}' }">
                            ...
                            <div class="flex flex-col sm:flex-row gap-4">
                                <label class="flex-1 relative cursor-pointer group" @click="currentRole = 'usuario'; togglePermisos(true)">
                                    <input type="radio" name="role" value="usuario" class="sr-only" {{ old('role', 'usuario') == 'usuario' ? 'checked' : '' }}>
                        ...
                        <div id="permisos-container" class="space-y-6 {{ old('role') == 'admin' ? 'hidden' : '' }}">
                                                ...
                                                <label class="flex items-start gap-3 group">
                                                    <div class="mt-0.5">
                                                        <input type="checkbox" name="permisos[]" value="{{ $codigo }}" 
                                                            ...
                                                            {{ in_array($codigo, old('permisos', [])) || $isDefault ? 'checked' : '' }}
                                                            {{ $isDefault ? 'disabled' : '' }}>
                                                        @if($isDefault)
    ...
    <script>
        function togglePermisos(show) {
            ...
            if (show) {
                ...
                // Opcional: desmarcar checkboxes si se pasa a Admin
                container.querySelectorAll('input[type="checkbox"]').forEach(cb => cb.checked = false);
            }
        }
        
        // Ejecutar al cargar la página por si había un viejo valor
        document.addEventListener('DOMContentLoaded', () => {
            const adminRadio = document.querySelector('input[name="role"][value="admin"]');
            if(adminRadio && adminRadio.checked) {
                togglePermisos(false);
            }
        });
    </script>
</x-app-layout>
